...
export default function DataImport({ entries, setEntries, quality, setQuality, importStatus, setImportStatus }) {
  ...
  const onFile = async (file) => {
    ...
    try {
      ...
      if (result.type === "production") {
        ...
        setMessage("לא הצלחתי לזהות אם זה קובץ ייצור/אריזה או איכות. נצטרך להתאים את מיפוי הכותרות.");
        return;
      }
  ...
  const clearProduction = () => {
    if (confirm("למחוק נתוני ייצור/אריזה שנטענו?")) {
      setEntries([]);
      setImportStatus({ ...importStatus, production: null });
    }
  };
  const clearQuality = () => {
    if (confirm("למחוק נתוני איכות שנטענו?")) {
      setQuality([]);
      setImportStatus({ ...importStatus, quality: null });
    }
  };

  return (
    <section className="page">
      <div className="page-head"><div><h2>טעינת נתונים</h2><p>טעינת Excel אחד לייצור/אריזה או לקובץ איכות. המערכת מזהה אוטומטית לפי כותרות הקובץ.</p></div><div className="version-badge">Sprint 3</div></div>
      ...
      <div className="panel">
        ...
        <div className="source-grid">
          <div className="source-card">
            ...
            <button className="secondary-btn" onClick={clearProduction}>נקה ייצור/אריזה</button>
          </div>
          <div className="source-card">
            <strong>איכות</strong>
            ...
            <button className="secondary-btn" onClick={clearQuality}>נקה איכות</button>
          </div>
        </div>
      </div>
    </section>
