...
export default function Quality({ quality }) {
  ...
  return <section className="page">
    ...
    <div className="panel"><div className="panel-title">פירוט איכות</div>{!mode ? <div className="empty-mini">לחץ על כרטיס כדי להציג רשומות</div> : rows.length ? <div className="table-wrap"><table><thead><tr><th>תאריך</th><th>מתקן</th><th>פקודה</th><th>Batch</th><th>חומר</th><th>Inspection Lot</th><th>סטטוס</th><th>מקור</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td>{r.date}</td><td>{r.facility}</td><td>{r.order}</td><td>{r.batch}</td><td>{r.material}</td><td>{r.inspectionLot}</td><td><span className={r.status==="ok"?"pill ok":r.status==="bad"?"pill bad":"pill warn"}>{r.status==="ok"?"תקין":r.status==="bad"?"חריג":"ממתין"}</span></td><td>{r.source}</td></tr>)}</tbody></table></div> : <div className="empty-mini">אין רשומות להצגה</div>}</div>
  </section>
}
